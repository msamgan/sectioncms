        ...
        >
            <a href="#" onClick={(e) => e.preventDefault()} className="flex items-center w-full justify-between">
                <div className="flex items-center">
                    ...
                    <div data-i18n={itemKey} className="whitespace-nowrap">
                        {itemKey}
                    </div>
                </div>
                <i
                    className={`ri-arrow-down-s-line ml-2 transition-transform duration-300 ${isOpen ? 'transform rotate-180 text-primary' : ''}`}
                ></i>
            </a>

            <ul
                className={`absolute left-0 top-full w-full mt-2 py-2 border border-light rounded-lg shadow-lg bg-white z-10 transition-all duration-300 ease-in-out ${isOpen ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-2 pointer-events-none'}`}
            >
                {menuItems[itemKey].map((item, index) => (
                    ...
                    >
                        <Link href={route(item.route)} className="flex items-center w-full">
                            ...
                            <div data-i18n={item.label} className="whitespace-nowrap">
                                {item.label}
                            </div>
                        </Link>
                    </li>
